...
//			schematicsFolder = findFolder(shapeFolder, generator.oreProvider.getCollectionName());
	...
	private File findFolder(File parent, String name) {
		...
		if (!result.isDirectory())
			if (!result.mkdir())
				throw new UnsupportedOperationException("[WorldEdit] Could not create/find the folder: "
						+ parent.getAbsolutePath() + File.separator + name);
		return result;
	}
					...
					try {
						...
						if (clip.chunkX > maxX || clip.chunkZ > maxZ) {
							generator.reportMessage("[WorldEdit] Schematic " + schematicFile.getName()
									+ " too large, max size = " + maxX * SupportBlocks.sectionBlockWidth + " by "
									+ maxZ * SupportBlocks.sectionBlockWidth + " it is = " + clip.sizeX + " by "
									+ clip.sizeZ + ", skipped");
		...
		try {
			...
			if (!isPlugInVersionOrBetter(generator, worldEditPlugin, pluginMinVersion))
				...
				if (generator.getSettings().forceLoadWorldEdit) {
					...
					generator.reportMessage("[PasteProvider] Cannot use the installed WorldEdit. ", "See the '"
							+ CityWorldSettings.tagForceLoadWorldEdit + "' setting for possible workaround.");
					return null;
				}
			...
			// woot!
			generator.reportMessage("[PasteProvider] Found WorldEdit v" + worldEditPlugin.getDescription().getVersion()
