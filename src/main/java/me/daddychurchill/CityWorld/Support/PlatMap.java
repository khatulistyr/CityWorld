...
//			}
	...
	public void validateRoads() {
		...
		if (!(isRoad(0, RoadLot.PlatMapRoadInset - 1) || isRoad(0, Width - RoadLot.PlatMapRoadInset)
				...
				|| isRoad(Width - 1, Width - RoadLot.PlatMapRoadInset) || isRoad(RoadLot.PlatMapRoadInset - 1, 0)
				|| isRoad(Width - RoadLot.PlatMapRoadInset, 0) || isRoad(RoadLot.PlatMapRoadInset - 1, Width - 1)
...
//				generator.reportMessage("Roundabout? " + generator.settings.includeRoundabouts + ", odds: " + context.oddsOfRoundAbouts + " context: " + context.toString());
				if (generator.getSettings().includeRoundabouts && generator.getSettings().inCityRange(originX + x, originZ + z)
						...
						&& isEmptyLot(x, z - 1) && isEmptyLot(x, z + 1) && isEmptyLot(x + 1, z - 1)
						&& isEmptyLot(x + 1, z) && isEmptyLot(x + 1, z + 1)) {
	...
	private boolean isRoadTowards(int x, int z, int deltaX, int deltaZ) {
		...
		// if this isn't a buildable spot, is there a bridge or tunnel that gets us
		// there?
		if (!result)
	...
	private boolean isBridgeTowards(int x, int z, int deltaX, int deltaZ) {
		...
		// short cut things a bit by looking for impossible things (polarity doesn't
		...
		for (int x = 0; x < chunksX; x++) {
			for (int z = 0; z < chunksZ; z++) {
				setLot(placeX + x, placeZ + z,
						new ClipboardLot(this, originX + placeX + x, originZ + placeZ + z, clip, facing, x, z));
			}
		}
	}
}
